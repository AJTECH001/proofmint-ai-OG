...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    // Allow common file types for receipts
    const allowedTypes = [
      'image/jpeg',
      'image/png',
      'image/gif',
...
/**
 * Upload multiple files to 0G Storage
 */
...
router.get('/download/:rootHash', async (req, res) => {
  try {
    ...
    const outputFilename = filename as string || `file_${Date.now()}`;
...
router.post('/receipt/:receiptId/attachment', upload.single('file'), async (req, res) => {
  try {
    ...
    const { buffer, originalname, mimetype } = req.file;
    const result = await storageService.uploadReceiptAttachment(
      buffer,
      originalname,
      receiptId,
      mimetype
...
router.get('/nodes/select', async (req, res) => {
  try {
    ...
    const segNum = segmentNumber ? parseInt(segmentNumber as string) : undefined;
    const replicas = expectedReplicas ? parseInt(expectedReplicas as string) : undefined;
    const excluded = excludedNodes ? (excludedNodes as string).split(',') : undefined;
...
/**
 * Upload file with enhanced node selection
 */
...
router.post('/upload-batch', upload.array('files', 20), async (req, res) => {
  try {
    ...
    try {
      // Write all files to temporary paths
...
router.get('/download-enhanced/:rootHash', async (req, res) => {
  try {
    ...
    const outputFilename = filename as string || `file_${Date.now()}`;
    const outputPath = path.join(downloadDir, outputFilename);
    const enableProof = withProof !== 'false'; // Default to true
