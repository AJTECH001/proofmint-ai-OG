...
export class ProofMintService {
  ...
  static async getAllReceipts(): Promise<Receipt[]> {
    const contract = await this.getContract();
    const receiptIds = await contract.viewAllReceipts();
  ...
  static async issueReceipt(
    ...
    const tx = await contract.issueReceipt(buyer, ipfsCID);
  ...
  static async getMerchantReceipts(merchantAddress: string): Promise<Receipt[]> {
    try {
      const contract = await this.getContract();
      const receiptIds = await contract.getMerchantReceipts(merchantAddress);
      ...
      console.error('Error in getMerchantReceipts:', error);
      return [];
    }
  }
  ...
  static async getBuyerReceipts(buyerAddress: string): Promise<Receipt[]> {
    try {
      const contract = await this.getContract();
      const receiptIds = await contract.getUserReceipts(buyerAddress);
      ...
      console.error('Error in getBuyerReceipts:', error);
      // Return empty array instead of throwing error
      return [];
    }
  }

  // NFC functionality is not available in the current contract

  // Recycler functions
  static async recycleGadget(receiptId: bigint): Promise<void> {
    const contract = await this.getContract();
    const tx = await contract.recycleGadget(receiptId);
    await tx.wait();
  }

  static async getRecyclerReceipts(recyclerAddress: string): Promise<Receipt[]> {
    const contract = await this.getContract();
    const receiptIds = await contract.getUserReceipts(recyclerAddress);
  ...
  static async getContractStats(): Promise<{
    ...
    const { totalReceipts } = await contract.getTotalStats();
