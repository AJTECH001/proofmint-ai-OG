...
const BuyerDashboard: React.FC = () => {
  ...
  const [nfcKeyHash, setNfcKeyHash] = useState<string | null>(null);
  
  // NFC linking form (functionality disabled)
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const buyerReceipts = await ProofMintService.getBuyerReceipts(address);
      
      setReceipts(buyerReceipts);
      setNfcKeyHash(null); // NFC functionality not available in current contract
    } catch (err) {
  ...
  const handleLinkNFC = async (e: React.FormEvent) => {
    e.preventDefault();
    alert("NFC functionality is not available in the current contract version.");
  };
